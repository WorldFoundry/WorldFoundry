...
QLevel::CreateAssetLST(const char* templatename)
{
	...
	for (unsigned int numRooms=0; numRooms < rooms.size()+1; numRooms++)
	...
	for(unsigned int roomIndex = 0; roomIndex < rooms.size()+1;roomIndex++)
	{
	...
	for(unsigned int objectIndex=0;objectIndex<objects.size();objectIndex++)
	{
		...
		if( (permFlagEntry && permFlagEntry->GetDef()) || (permFlagEntry == NULL) )
		{
			...
			for (unsigned int oadIndex = 0;oadIndex < objOad.entries.size(); oadIndex++)
			{
                ...
                if (oadEntry.GetType() == BUTTON_FILENAME)
				{
					...
					if (oadEntry.GetString().length() == 0)
					{
						...
						if (modelType != NULL)
						{
							...
							{
								AssertMessageBox(0, "Required asset " << oadEntry.GetName() << " missing from object " << thisObjectName << ", object type is " << objectTypes[obj->GetTypeIndex()]);
							}
						}
					}
					else
...
QLevel::PatchAssetIDsIntoOADs()
{
	...
	for(unsigned int roomIndex = 0; roomIndex < rooms.size();roomIndex++)
	{
		for(int objectIndex = 0;objectIndex < rooms[roomIndex].length(); objectIndex++)
		{
			...
			{
				QObjectAttributeDataEntry& oadEntry = (QObjectAttributeDataEntry&)objOad.entries[oadIndex];
                if ( (oadEntry.GetType() == BUTTON_FILENAME) && oadEntry.GetString().length() )
				{
					...
					for(map<string,string,less<string> >::iterator iter(extensionsMap.begin());iter != extensionsMap.end();iter++)
						if(ext = strstr(oldName,(*iter).first.c_str()))
						{
...
//	packedID.Type(assetExts.Lookup((const char*)strrchr(writableName, '.')));
	packedID.Type(assetExts.Lookup(string(extension)));

	packedID.Index(assetIndices[roomNum][packedID.Type()]);
	assetIndices[roomNum][packedID.Type()] = assetIndices[roomNum][packedID.Type()]+1;
...
QLevel::SaveAssetLSTFile(const char* templatename)
{
	...
	DBSTREAM3( cdebug << " asset.PRP filename = " << result << endl; )
	ofstream assetlst(result);
	AssertMessageBox(assetlst.good(), "Cannot open file <" << result << "> for write");

	assetlst << "@*============================================================================" << endl;
	assetlst << "@* asset.prp: asset list for use with prep, created by LevelCon V"LEVELCON_VER", DO NOT MODIFY" << endl;
	assetlst << "@*============================================================================" << endl;
