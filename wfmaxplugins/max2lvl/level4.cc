...
// now load all of the .oad files for all of the objects
	...
	for(unsigned int i=1;i<objectTypes.size();i++)		// note: this starts at one since 0 is the null object
	{
		...
		ifstream input(line.c_str(),ios::in|ios::binary|ios::nocreate);
...
// ok, verify it is an .lc file
	...
	AssertMessageBox(lineBuffer.find("Objects.lc") != NPOS, name << endl << "is not a valid .LC file!");
...
//	fwrite(&diskLevel,sizeof(_LevelOnDisk),1,fp);
	...
	assert(lvl.tellp() == diskLevel.objectsOffset);
	int32 tempOffset;
...
//		fwrite(&tempOffset,sizeof(int32),1,fp);
	...
	assert(lvl.tellp() == diskLevel.pathsOffset);
...
//		fwrite(&tempOffset,sizeof(int32),1,fp);
	...
	assert(lvl.tellp() == diskLevel.channelsOffset);
...
//		fwrite(&tempOffset,sizeof(int32),1,fp);
	...
	assert(lvl.tellp() == diskLevel.roomsOffset);
...
QLevel::CreateQObjectList( /*SceneEnumProc* theSceneEnum*/ )
{
	...
	for ( int i=0; i<_pRoot->NumberOfChildren(); ++i )
	{
		...
		DBSTREAM3( cdebug << "QLevel::CreateQObjectList():  Processing node named <" << pObject->GetName() << ">" << endl; )
		ObjectState os = pObject->EvalWorldState( 0 );
		...
		if ( os.obj->SuperClassID() == GEOMOBJECT_CLASS_ID )
		{
			...
			if ( mesh )
			{
				...
				Matrix3 objTM = pObject->GetObjectTM(theTime);
				Point3 location = objTM.GetTrans() / masterScale;

				DBSTREAM3( cstats << "processing mesh " << pObject->GetName() << endl << "At " << )
					DBSTREAM3( location.x << "," << )
					DBSTREAM3( location.y << "," << )
					DBSTREAM3( location.z << endl; )

				CreateQObjectFromSceneNode( mesh, pObject, time );
				DBSTREAM3( cstats << "Done processing mesh " << endl;)
			}
		}
		else if ( os.obj->SuperClassID() == LIGHT_CLASS_ID )
		{
			...
			DBSTREAM3( cstats << "processing light " << pObject->GetName() << endl << "At " << )
				DBSTREAM3( location.x << "," << )
				DBSTREAM3( location.y << "," << )
				DBSTREAM3( location.z << endl; )

			CreateQObjectFromSceneNode( NULL, pObject, time );
		}

	}

#if 0
	SceneEntry* thisEntry = theSceneEnum->head;

	while (thisEntry != NULL)
	{
		DBSTREAM3( cdebug << "QLevel::CreateQObjectList():  Processing node named <" << thisEntry->node->GetName() << ">" << endl; )

		ObjectState os = thisEntry->node->EvalWorldState(theSceneEnum->time);
		...
		if ( os.obj->SuperClassID() == GEOMOBJECT_CLASS_ID )
		{
			BOOL needDel;
			NullView nullView;
			Mesh* mesh = ((GeomObject*)os.obj)->GetRenderMesh(theSceneEnum->time, thisEntry->node, nullView, needDel);
			if (mesh)
			{
				float masterScale = GetMasterScale(UNITS_METERS);
				Matrix3 objTM = thisEntry->node->GetObjectTM(theTime);
				Point3 location = objTM.GetTrans() / masterScale;

				DBSTREAM3( cstats << "processing mesh " << thisEntry->node->GetName() << endl << "At " << )
					DBSTREAM3( location.x << "," << )
					DBSTREAM3( location.y << "," << )
					DBSTREAM3( location.z << endl; )

				CreateQObjectFromSceneNode(mesh, thisEntry->node, theSceneEnum->time);
			}
		}
		else if (os.obj->SuperClassID()==LIGHT_CLASS_ID)
		{
			...
			DBSTREAM3( cstats << "processing light " << thisEntry->node->GetName() << endl << "At " << )
				DBSTREAM3( location.x << "," << )
				DBSTREAM3( location.y << "," << )
				DBSTREAM3( location.z << endl; )

			CreateQObjectFromSceneNode(NULL, thisEntry->node, theSceneEnum->time);
		}

		thisEntry = thisEntry->next;
	}
#endif
	DBSTREAM1( cprogress << "Done Creating QObject list from the current Scene." << endl; )
}
