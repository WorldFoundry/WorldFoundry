...
BOOL D3DTexture::Load(IDirect3DDevice2 *Device, char *BitmapName)
{
    BITMAP              bm;
    DDSURFACEDESC       ddsd;
    HBITMAP             Bitmap;
...
/**************************************************************************
 ...
 create a D3D texture map

 **************************************************************************/

BOOL D3DTexture::Create(IDirect3DDevice2 *Device, int xSize, int ySize, int bitsPerPixel)
{
    DDSURFACEDESC       ddsd;
    HBITMAP             Bitmap;
    IDirectDraw *       DirectDraw;

    DPF("D3DTexture::Create(%d,%d)", xSize, ySize);
    ...
    ZeroMemory(&ddsd, sizeof(ddsd));
    ddsd.dwSize = sizeof(ddsd);
    ChooseTextureFormat(Device, bitsPerPixel, &ddsd.ddpfPixelFormat);
    ddsd.dwFlags |= DDSD_CAPS | DDSD_HEIGHT | DDSD_WIDTH | DDSD_PIXELFORMAT;
    ddsd.dwWidth = xSize;
    ddsd.dwHeight = ySize;
    ...
    {
        // when dealing with a SW rasterizer we dont need to make two surfaces.
        MemorySurface = DeviceSurface;
        DeviceSurface->AddRef();
    }
#endif
    ...
    if (ddsd.ddpfPixelFormat.dwRGBBitCount <= 8)
    {
        ...
        // now we have a palette, attach the palette to the Surface and the texture
        MemorySurface->SetPalette(Palette);
        DeviceSurface->SetPalette(Palette);
    }
