 ...
 */
long
maudread(ft_t ft, long* buf, long len)
{
	struct maudstuff * p = (struct maudstuff *) ft->priv;
	register int datum;
	int done = 0;
 ...
 */
void
maudstopread(ft_t ft)
{
}


void
maudstartwrite(ft_t ft)
{
	struct maudstuff * p = (struct maudstuff *) ft->priv;
	int littlendian = 0;
	char *endptr;
	...
	if (ft->info.size == cbWORD) ft->info.style = SIGN2;
	if (ft->info.style == ULAW || ft->info.style == ALAW) ft->info.size = cbBYTE;
	if (ft->info.size == cbBYTE && ft->info.style == SIGN2) ft->info.style = UNSIGNED;
...
void
maudwrite(ft_t ft, long* buf, long len)
{
	struct maudstuff * p = (struct maudstuff *) ft->priv;
