...
void compress(unsigned char* * df, long* dl, float fr);
void padbytes(ft_t ft, int n);

void
skipbytes(ft_t ft, int n)
{
	while (--n >= 0) {
		if (getc(ft->fp) == EOF)
...
long
hcomread(ft_t ft, long* buf, long len)
{
...
void
hcomwrite(ft_t ft, long* buf, long len)
{
	register struct writepriv *p = (struct writepriv *) ft->priv;
	long datum;
...
void
putlong(unsigned char* c, long v)
{
  *c++ = (v >> 24) & 0xff;
  *c++ = (v >> 16) & 0xff;
  *c++ = (v >> 8) & 0xff;
...
void putshort(unsigned char* c, short v)
{
  *c++ = (v >> 8) & 0xff;
  *c++ = v & 0xff;
}
...
void makecodes(int e, int c, int s, int b)
{
...
void putcode(unsigned char c, unsigned char* * df)
{
long code, size;
int i;
  code = codes[c];
...
void
compress(unsigned char* * df, long* dl, float fr)
{
  long samplerate;
