 ...
 */
void
update(int code_size, int y, int wi, int fi, int dq, int sr, int dqsez, struct g72x_state* state_ptr)
#if 0
...
{
	int		cnt;
	short		mag, exp, mant;	/* Adaptive predictor, cbFLOAT A */
	short		a2p;		/* LIMC */
	short		a1ul;		/* UPA1 */
	short		ua2, pks1;	/* UPA2 */
	short		uga2a, fa1;
	...
	/* cbFLOAT A : convert dq[0] to 4-bit exp, 6-bit mantissa f.p. */
	if (mag == 0) {
	...
	/* cbFLOAT B : convert sr to 4-bit exp., 6-bit mantissa f.p. */
	if (sr == 0) {
		state_ptr->sr[0] = 0x20;
	} else if (sr > 0) {
 ...
 */
int
tandem_adjust_alaw(int sr, int se, int y, int i, int sign, short* qtab)
#if 0
...
int
tandem_adjust_ulaw(int sr, int se, int y, int i, int sign, short* qtab)
#if 0
	int		sr;	/* decoder output linear PCM sample */
	int		se;	/* predictor estimate sample */
	int		y;	/* quantizer step size */
