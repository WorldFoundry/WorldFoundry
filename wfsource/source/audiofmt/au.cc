...
#define SUN_cbFLOAT	6			/* cbIEEE FP 32 bits */
#define SUN_cbDOUBLE	7			/* cbIEEE FP 64 bits */
...
void auwriteheader( ft_t ft, unsigned long data_size );
 ...
 */
int
unpack_input(ft_t ft, unsigned char* code)
{
	struct aupriv		*p = (struct aupriv *) ft->priv;
...
long
auread(ft_t ft, long* buf, long samp)
{
	struct aupriv *p = (struct aupriv *) ft->priv;
	unsigned char code;
	int done;
...
void
auwrite(ft_t ft, long* buf, long samp)
{
	struct aupriv *p = (struct aupriv *) ft->priv;
...
void
auwriteheader( ft_t ft, unsigned long data_size )
{
	unsigned long magic;
	unsigned long hdr_size;
	unsigned long encoding;
	...
	if (ft->info.style == ULAW && ft->info.size == cbBYTE)
		encoding = SUN_ULAW;
	else if (ft->info.style == SIGN2 && ft->info.size == cbBYTE)
		encoding = SUN_LIN_8;
