...
long pred_init(eff_t effp, long* ibuf, long* obuf);
long pred_ict(eff_t effp);
void pred_next(eff_t effp, long samp);

/*
 * Process options
 */
void
pred_getopts(eff_t effp, int n, char* argv[] )
{
...
void
pred_flow(eff_t effp, long* ibuf, long* obuf, int* isamp, int* osamp)
{
	int len, done;
	pred_t pred = (pred_t) effp->priv;
 ...
 */
long
pred_init(eff_t effp, long* ibuf, long* obuf)
{
	pred_t pred = (pred_t) effp->priv;
 ...
 */
void
pred_next(eff_t effp, long samp)
{
	pred_t pred = (pred_t) effp->priv;
