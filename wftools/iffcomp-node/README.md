...
Node.js port of [iffcomp](../iffcomp/), the WorldFoundry IFF compiler.
...
[Rust](../iffcomp-rs/) versions.

## Usage

```
node src/main.js [-o=<file>] [-binary|-ascii] [-v] [-q] <input.iff.txt>
```
...
const bytesWritten = compile(
  'input.iff.txt',
  'output.iff',
  {
    mode:    Mode.BINARY,   // or Mode.TEXT
    verbose: false,
    cwd:     '/base/dir',   // for relative includes / file refs (default: dir of inFile)
  }
);
```
...
```
node --test test/byte_exact.test.js
```

Three byte-exact comparisons against the C++ oracle binary stored in
`testdata/` (shared with the Go and Rust ports). The `.timestamp` chunk
payload is masked before comparison because it is time-varying.

## Implementation

| File | Purpose |
|------|---------|
| `src/errors.js` | `IffError` with kind, file, line, col |
| `src/lexer.js` | Hand-rolled tokenizer with include stack |
| `src/parser.js` | Recursive-descent parser, state stack, back-patch queue |
| `src/writer.js` | Binary + text output, fixed-point encoding |
| `src/iffcomp.js` | `compile()` library entry point |
| `src/main.js` | CLI wrapper |

No dependencies. Node.js ≥ 20 required.
