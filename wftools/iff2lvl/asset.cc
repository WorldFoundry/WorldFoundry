...
{
	DBSTREAM3( cdebug << "AssetExtensions::Load: " << std::endl; )
		...
		if(strlen(stringBuffer) != 0 && (stringBuffer[0] != '/' && stringBuffer[1] != '/'))
		 {
			...
			DBSTREAM3( cdebug << "AssetExtensions::Load: parsing line <" << stringBuffer << ">" << std::endl; )
//			assert(Lookup(stringBuffer) == -1);				// insure it is not already in the list
			extensions.push_back(stringBuffer);
		 }
	 }
	DBSTREAM3( cdebug << "AssetExtensions::Load: list:" << std::endl; )
...
AssetExtensions::Lookup(const string& inputExtension)
{
	...
	DBSTREAM3( cdebug << "AssetExtensions::Lookup: string = <" << extension << ">" << std::endl; )

	for(unsigned int index=0;index<extensions.size();index++)			// kts does STL have a better way to do this?
	 {
		DBSTREAM3( cdebug << "AssetExtensions::Lookup: checking index " << index << " which contains " << extensions[index] << std::endl; )
		if(extensions[index] == extension)
		 {
			DBSTREAM3( cdebug << "found, index = " << index << std::endl; )
			return(index);
		 }
	 }
