...
//						tempBlockSize++;
					...
					case BUTTON_STRING:
						//assert(0);
                  {
                     DBSTREAM3( cdebug << "CCD: BUTTON_STRING: saving 4-byte asset ID <" << hex << td->GetDef() << dec << ">" << endl; )
                     int32 tempVal = td->GetDef();
                     memcpy(&tempBlock[tempBlockSize], (char*)&tempVal, 4);
                     tempBlockSize += 4;
                     break;
						...
						if (tempBlockSize > 0)				// skip if no common data
						 {
							...
							DBSTREAM3( cdebug << "QLevel::CreateCommonData: ending common block,  entry set  to" << *temp_td << endl; )
	...
	if(rooms.size() == 0)
	 {
		...
		QColBox roomBox
		(
			Point3(-32767.0,-32767.0,-32767.0),
			Point3(32767.0,32767.0,32767.0)
		);
