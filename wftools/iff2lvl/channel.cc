//============================================================================
//	channel.hp:	Encapsulates a parameterized stream of values
//
...
ostream& operator << ( ostream& out , const ChannelEntry& ce)
{
	out << "ChannelEntry: time = " <<  ce.time << ", value = " << hex << ce.value;
	return out;
}
...
Channel::AddKey(ChannelEntry newEntry)
{
	...
	for (entryIndex=0; entryIndex < _entries.size(); entryIndex++)
		AssertMsg( _entries[entryIndex].time != newEntry.time, "newEntry = " << newEntry << ", current entry = " << _entries[entryIndex] );
#endif
...
//	thisChannelOnDisk->endTime = WF_FLOAT_TO_SCALAR( (_entries.end() - 1)->time / TICKS_PER_FRAME );
	thisChannelOnDisk->endTime = (_entries.end() - 1)->time;
...
//		entryPtr[index].time  = WF_FLOAT_TO_SCALAR( _entries[index].time / TICKS_PER_FRAME );
		entryPtr[index].time  = _entries[index].time;
