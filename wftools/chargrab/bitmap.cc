//==============================================================================
// bitmap.cc: Copyright (c) 1996-1999, World Foundry Group  
...
//		*log << name << " [" << _startColour << ".." << _endColour << ']' << endl;
...
#pragma pack()
	...
	if ( bFlipYOut )
	{
		...
		{
			uint16* pixeldata = (uint16*)pixels + (y * _bufWidth);
			int cbWritten = fwrite( pixeldata, sizeof( uint16 ), th.Width, fp );
	...
	for ( int i=0; i<_nColourCycles; ++i )
	{
		...
		assert( cbWritten == 1 );
		int32 speed = cc._speed * 65536.0;		// convert to fixed point
	...
	const char* szRoomName, const char* szTextureType, int i )
{
	...
	if ( !bFoundSlot )
	{
		...
		*log << error() << "couldn't fit " << szTextureType <<  " \"" << bmAdd->name << "\" in room \"" << szRoomName << "\"" << endl;
		bGlobalError = true;
...
void
Bitmap::copy(
	const Bitmap* srcBitmap, const int sourceXPos, const int sourceYPos, const int xSize, const int ySize,
	                         const int destXPos, const int destYPos)
{
	...
	assert( ( srcBitmap->_cbRow % 2 ) == 0 );
	RangeCheckInclusive(sourceXPos,0,srcBitmap->width());
	RangeCheckInclusive(sourceXPos+xSize,0,srcBitmap->width());
	RangeCheckInclusive(sourceYPos,0,srcBitmap->height());
	RangeCheckInclusive(sourceYPos+ySize,0,srcBitmap->height());
	...
	switch ( srcBitmap->bitdepth() )
	{
		...
		{
			assert(0);
			for ( int y=sourceYPos; y<sourceYPos+ySize; ++y )
			{
				memcpy( ((uint16*)pixels) + (y + destYPos) * _cbRow/2 + destXPos,
					(int8*)srcBitmap->pixels + y * srcBitmap->_cbRow,
					srcBitmap->_cbRow );
			}
			break;
		...
		case 16:
		{
			...
			{
				uint16* dest = ((uint16*)pixels);
				dest +=	((y + destYPos) * _width);
				dest +=	destXPos;
				memcpy( dest,
					((uint16*)srcBitmap->pixels) +
						((y+sourceYPos) * srcBitmap->width()) +
						sourceXPos,
					xSize*sizeof(uint16) );
			}
		}
